...
while [[ ! $(ssh -o BatchMode=yes -o ConnectTimeout=5 192.168.200.100 echo connected 2>&1) =~ "connected" ]]; do
	sleep 5
...
ssh-keyscan -H 192.168.200.100 >> ~/.ssh/known_hosts
ssh 192.168.200.100 "ssh-keygen -q -t rsa -N '' -f ~/.ssh/id_rsa <<<y >/dev/null 2>&1"
ssh 192.168.200.100 "ssh-keyscan -H 192.168.200.1  >> ~/.ssh/known_hosts"
ssh -q 192.168.200.100 "cat ~/.ssh/id_rsa.pub" >> ~/.ssh/authorized_keys

echo "----------------------------------------------------------------------"
echo
echo "Now you will be sshing to helper node 192.168.200.100."
